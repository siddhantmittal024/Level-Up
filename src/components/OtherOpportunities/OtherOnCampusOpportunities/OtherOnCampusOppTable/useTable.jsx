...
const EnhancedTable = ({ tableHeader }) => {
  ...
  const getOtherOnCampusOpportunities = async () => {
    //console.log('hello');
    try {
  ...
  const handleDelete = async (e, opportunityId) => {
    e.preventDefault();
    setLoading(true);
    deleteDoc(doc(db, 'other-on-campus-opportunities', opportunityId))
      .then(async () => {
        getOtherOnCampusOpportunities();
      })
      .catch((err) => {
  ...
  const printEditIcon = (userID, opportunityId) => {
    if (userID === user.uid) {
      return (
        <Link to={`/other-opportunities/on-campus/update/${opportunityId}`}>
          <Tooltip title="Edit">
                ...
                .map((opportunity, index) => {
                  return (
                    <TableRow hover key={opportunity.id}>
                      ...
                      <TableCell>
                        <Link
                          to={`/other-opportunities/on-campus/${opportunity.id}`}
                          style={{
                            textDecoration: 'none',
                            width: '100%',
                            color: 'black'
