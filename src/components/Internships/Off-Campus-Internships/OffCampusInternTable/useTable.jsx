...
const EnhancedTable = ({ tableHeader }) => {
  ...
  const internsRef = collection(db, 'off-campus-internships');
  ...
  const getFilteredRows = () => {
    ...
    return rows.filter((intern) =>
      intern.companyName.toLowerCase().includes(searched.toLowerCase())
    );
  };
  ...
  const handleDelete = async (e, internId) => {
    e.preventDefault();
    setLoading(true);
    deleteDoc(doc(db, 'off-campus-internships', internId))
      .then(async () => {
        getInternships();
      })
      .catch((err) => {
  ...
  const printEditIcon = (userID, internID) => {
    if (userID === user.uid) {
      return (
        <Link to={`/internships/off-campus/update/${internID}`}>
          <Tooltip title="Edit">
  ...
  const printDeleteIcon = (userID, internID) => {
    if (userID === user.uid) {
      return (
        <Tooltip title="Delete">
          <DeleteIcon
            sx={{ color: 'red', cursor: 'pointer' }}
            onClick={(e) => handleDelete(e, internID)}
          />
        </Tooltip>
      );
    } else {
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        >
          ...
          <Link
            ...
            to={'/internships/off-campus/add-internship'}
          >
            <Button
              variant="contained"
              color="primary"
          ...
          >
            ...
            <TableBody>
              {stableSort(getFilteredRows(), getComparator(order, orderBy))
                ...
                .map((intern, index) => {
                  return (
                    <TableRow hover key={intern.id}>
                      <TableCell>{intern.tagline}</TableCell>
                      <TableCell>{intern.companyName}</TableCell>
                      <TableCell>{intern.position}</TableCell>
                      <TableCell>{intern.batch}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Link
                          to={`/internships/off-campus/${intern.id}`}
                          style={{
                            textDecoration: 'none',
                            width: '100%',
                            color: 'black'
