...
const EnhancedTable = ({ tableHeader }) => {
  ...
  const internshipsRef = collection(db, 'on-campus-internships');
  // const getJobs = async () => {
  //   try {
  ...
  const getInternships = async () => {
    ...
    try {
      ...
      //console.log(expiredInternships);
      setRows(expiredInternships);
      setLoading(false);
    } catch (err) {
      console.log(err);
      setError(err);
  ...
  const handleDelete = async (e, jobId) => {
    e.preventDefault();
    setLoading(true);
    deleteDoc(doc(db, 'on-campus-internships', jobId))
      .then(async () => {
        getInternships();
      })
      .catch((err) => {
                ...
                .map((job, index) => {
                  return (
                    <TableRow hover key={job.id}>
                      ...
                      <TableCell>
                        <Link
                          to={`/internships/on-campus/${job.id}`}
                          style={{
                            textDecoration: 'none',
                            width: '100%',
                            color: 'black'
